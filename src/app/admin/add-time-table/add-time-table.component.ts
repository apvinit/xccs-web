...
export class AddTimeTableComponent implements OnInit {
  ...
  uploadFile(event):any{
    ...
    const filePath = 'Timetable/' + this.prog_name.toUpperCase() + "/" + this.timetable_type + "/" + "sem_" + this.semester;
    ...
    this.task.snapshotChanges().pipe(
        finalize(() => {
          ...
          this.downloadURL.subscribe((url) => this.timetableForm.patchValue({url : url}));
        })
     )
    .subscribe();
  ...
  onSubmit(){
    let id = this.timetableForm.get('prog_id').value;
    let data = {
